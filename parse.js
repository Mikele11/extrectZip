...
zip.on('error', err => { console.log('Oops! Something bad') });
zip.on('ready', () => {
	if (fs.existsSync('extracted')) {
    console.log('update dir extracted');
	}else {
    ...
    zip.extract(null, './extracted', (err, count) => {
		...
		function parser(file){
			var data = fs.readFileSync(path.join(__dirname, 'extracted/'+file), { encoding : 'utf8'});
			var options = {
			  delimiter : ',', // optional 
			  quote     : '"' // optional 
			};
